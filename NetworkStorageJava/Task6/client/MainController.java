...
public class MainController implements Initializable {
                ...
                while (true) {
                    ...
                    if (abstractMessage instanceof AuthMessage) {
                        ...
                        if (authMessage.getMessage().equals("True")) {
                            ...
                            Platform.runLater(() -> label.setText("Неверный логин или пароль"));
                        }
                        else if (authMessage.getMessage().equals("FalseLogin")) {
                            Platform.runLater(() -> label.setText("Логин уже занят"));
                        }
                    }
                }
    ...
    public void downloadFile(ActionEvent actionEvent) {
        sendMessage(new DownloadRequest(serverListView.getSelectionModel().getSelectedItem()));
    }

    public void uploadFile(ActionEvent actionEvent) {
        try {
            sendMessage(new FileMessage(Paths.get("client/repository/" + clientListView.getSelectionModel().getSelectedItem())));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void deleteFile(ActionEvent actionEvent) {
        if (clientListView.getSelectionModel().getSelectedItem() != null) {
            ...
            sendMessage(new DeleteRequest(serverListView.getSelectionModel().getSelectedItem()));
        }
    }
